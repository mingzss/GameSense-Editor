...
void main()
{
    vec4 oClr = texture(uTex2dArray, vec3(gs_texcoord, gl_Layer));
    ...
    if (tempSettings.z >= 0.0f)
    {
        ...
        float originalLuminance = Luminance(oClr.rgb);
        
        vec3 blended = mix(oClr.rgb, oClr.rgb * colorTempRGB, factor);
        ...
        fs_color = 
		    vec4(mix(blended, luminancePreservedRGB, LUMINANCE_PRESERVATION), 
                1.0);
    }
